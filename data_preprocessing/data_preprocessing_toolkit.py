...
class DataPreprocessingToolkit(object):
    ...
    def add_night_price(self, df):
        # Write your code here - done
        df.loc[:, "night_price"] = ((df["accomodation_price"] / df["length_of_stay"]) / df["n_rooms"]).round(decimals = 2)
    ...
    def map_night_price_to_room_segment_buckets(self, df):
        ...
        night_prices.columns = ['room_group_id', 'avg_night_price']
        df = pd.merge(df, night_prices, on=['room_group_id'], how='left')
        df.loc[:, 'room_segment'] = df['avg_night_price'].apply(lambda x: self.map_value_to_bucket(x, self.room_segment_buckets))
        df.drop(columns=["avg_night_price"])
